...
@section('content')
	<div id="interactive" class="dash-panel">
		<div class="panel-body text-center">
			...
			<ul id="ingredient-box" class="interactiveLists">
				...
				@foreach($steps as $k=>$s)
					<li>
						<div class="item">
							<a href="#">
								<img src="{{ asset('img/interactive/fries/'.($s['thumb']).'') }}" alt="{{ str_slug($s['step']) }}" data-swf="{{ asset('img/interactive/fries/'.($s['swf']).'') }}" class="img-responsive" data-step="{{ ($k+1) }}">
							</a>
			...
			<div id="assembly-tray" style="background-image:url('{{ asset('img/interactive/fries/stage.jpg') }}')">
			</div>

			<form id="submission-form" action="/form/saveAnswerInteractivePracticefries" method="POST">
				{!! csrf_field() !!}
				<input type="hidden" name="code" value="fries">
